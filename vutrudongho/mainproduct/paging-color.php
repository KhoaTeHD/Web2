<div id="main">
    ...
    $total = $conn->prepare("select * from product where Color = ?");
    $total->bind_param("s", $url);
    $total->execute();
    $total_a = $total->get_result();
    $total_a = $total_a->num_rows;
    $total_page = ceil($total_a / $item_page);
    $stmt = $conn->prepare("select * from product where Color = ? order by ProductID asc LIMIT " . $item_page . " OFFSET " . $offset);
