            ...
            <?php for ($num = 1; $num <= $total_page; $num++) { ?>
                <?php if ($num != $cur_page) { ?>
                    <?php if ($num > $cur_page - 2 && $num < $cur_page + 2) { ?>
                        <a class="page-item"
                            href="?page=<?= $num ?><?php echo ($search != '') ? "&keyword=$search" : '' ?><?php echo ($brand != '') ? "&brandname=$brand" : '' ?><?php echo ($pricefrom != '') ? "&from-advanced=$pricefrom" : '' ?><?php echo ($priceto != '') ? "&to-advanced=$priceto" : '' ?>&nang-cao=Tìm+kiếm"><?= $num ?></a>
                    <?php } ?>
                <?php } else { ?>
